...
//matrix plus, negate matrix, sub matrix, divide by integer, times an integer(2 different), times another matrix(matrixproduct) , matrix times vector(need two?), 
	...
	public static matrix operator*(matrix a, matrix b){
 System.Console.WriteLine("fucking pisbræk \n");
 for(int i=0;i<a.colsize;i++) a[i].print();
 System.Console.WriteLine("fucking pislort \n");
 for(int i=0;i<b.colsize;i++) b[i].print();
   if (a.rowsize != b.colsize)  
    throw new InvalidOperationException  
      ("Product is undefined. n columns of first matrix must equal to n rows of second matrix");
	var r = new matrix(a.rowsize,b.colsize);
 
 System.Console.WriteLine($"jeg hader det her lort, {a.rowsize} og {b.colsize} \n"); 
 //ALT VEL SO FAR
 ...
 r[i][j]=a[i][k]*b[k][j];
 System.Console.WriteLine($"rij={r[i,j]} \n");
 }
 }
 }
 return r;
...
*/
	...
	public matrix copy(){
		matrix r = new matrix(this.rowsize,this.colsize);
